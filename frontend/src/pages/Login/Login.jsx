...
export default function Login() {
  ...
  const validate = () => {
    const next = {}
    if (!form.username.trim()) next.username = 'Enter your Student ID or Staff ID.'
    ...
    <div className="d-flex flex-column flex-lg-row vh-100 overflow-hidden">
      ...
      >
        ...
        <div>
          ...
          <h1 className="text-uppercase lh-1 fw-semibold" style={{ ...fontDisplay, fontSize: 'clamp(2.4rem, 4vw, 3.4rem)' }}>
            <span className="d-block">BACK ON</span>
            <span className="d-block">THE COURT.</span>
          </h1>
          <p className="fs-5 opacity-75 mt-3" style={{ maxWidth: '30ch' }}>
            Pick up your streak, check today&rsquo;s steps, or book your next slot.
          </p>
        </div>

        <span className="small opacity-50" style={fontMono}>
          ONE LOGIN &middot; STUDENTS &amp; STAFF
        </span>
      </div>
      ...
      <div className="flex-grow-1 d-flex align-items-center overflow-auto">
        <div className="p-4 p-lg-5 mx-auto w-100" style={{ maxWidth: '28rem' }}>
          ...
          <p className="text-body-secondary mb-4">One login for both students and staff.</p>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <p className="text-center small text-body-secondary mt-3 mb-0">
              New here?{' '}
              <button type="button" className="btn btn-link btn-sm p-0 align-baseline" onClick={() => navigate('/register')}>
                Create an account
              </button>
            </p>
          </form>
